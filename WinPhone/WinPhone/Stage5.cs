...
namespace WinPhone
{
    public partial class Stage5 : Form
    {
        ...
        public Stage5()
        {
            ...
            Question.Text = quiz.Question5[num, 0];
            Hint1.Text = quiz.Hint5[num, 0];
            Hint2.Text = quiz.Hint5[num, 1];
        ...
        private void Complete_Click(object sender, EventArgs e)
        {
            try
            {
                if (quiz.Question5[num, 1] == Input.Text)        // 문제와 입력받을 값을 비교
                {
                    num++;                                      // 문제 정답을 맞췄을 때 문제 번호 1 증가
                    MessageBox.Show("정답입니다!!");
                    if (num < 5)
                    {
                        Question.Text = quiz.Question5[num, 0]; // 다음 문제로 넘어가기
                        Hint1.Text = quiz.Hint5[num, 0];        // 다음 힌트로 넘어가기
                        Hint2.Text = quiz.Hint5[num, 1];        // 다음 힌트로 넘어가기
